...
## at enhancers has on RNA expression
#******************************************************
...
# Overlap between enhancer regions and cCREs
# Proximal enhancers are chosen
# Uncomment rE2G to change to the distal enhancers
enhancer_ccres_overlap <-
  vroom(
    "data/enhancers/proximal_enhancers_cCREs_intersection.tsv",
    #"data/enhancers/rE2G_enhancers_cCREs_intersection.tsv",
    col_names = FALSE,
...
# Join the `enhancer_ccres_overlap` table with the `thmm_profiles` table
# to get a gene assignment for every cCRE with a state profile
thmm_profiles_per_gene <-
  enhancer_ccres_overlap %>%
  separate_wider_position(X14, c(gene_id = 15), too_many = "drop") %>%
  inner_join(
    thmm_profiles,
    by = c("X7" = "cCRE")
  )
...
# Choose genes whose enhancer transition
...
# Control genes are the ones whose enhancer remains
...
# Test genes are the ones whose enhancer undergoes
...
to_test <-
  ...
  summarise(n_genes = n()) %>%
  arrange(desc(n_genes)) %>%
  filter(n_genes >= 20 & transition != "3 -> 6") %>%
  mutate(origin_state = str_sub(transition, 1, 1)) %>%
  left_join(
    control_genes %>%
      left_join(gene_profiles_longer) %>%
      select(gene_id, state) %>%
      distinct() %>%
      group_by(state) %>%
      summarise(n_genes_control = n()) %>%
      mutate(state = as.character(state)),
    by = c("origin_state" = "state")
  ) %>%
  filter(n_genes_control >= 20) %>%
  pull(transition)
