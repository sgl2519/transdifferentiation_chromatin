...
cat $peaks | while read file; do
	...
	awk 'BEGIN { FS=OFS="\t" }{if ( $NF==0 ) {print $1, $2, $3, $7"_"$4, $5, $6 } else {start = ($2>$9)?$2:$9; end = ($3<$10)?$3:$10; print $1, start, end, $7"_"$4, $5, $6 }}' > "$(basename $file .bed.gz)".coordinates.intersection.bed
		
		
	if [[ "$n" == 1 ]]
	then		
		echo $(basename $file .bed.gz)".coordinates.intersection.bed" > "$target".path.coordinates.txt
	else
		echo $(basename $file .bed.gz)".coordinates.intersection.bed" >> "$target".path.coordinates.txt
	fi
done
...
for i in `seq 1 $n`; do
	...
	/users/rg/bborsari/bwtool/bwtool summary -header -with-sum -keep-bed $bed_coordinates $file "$outFolder"/"$(basename $file .bigWig)".all.genes.summary.tsv
	if [[ "$i" == 1 ]]
	then 
...
/users/rg/bborsari/bin/selectColumns.sh name:"$signal" "$file1" | tail -n+2 |awk 'BEGIN{FS=OFS="\t"}{split($1, a, "_"); print a[1], $2}' | sort -k1,1 -k2,2gr | sort -u -k1,1 > "$outFile"
...
tail -n+2 "$target".path.bwtool.summary.txt | while read file; do
	...
	/users/rg/bborsari/bin/selectColumns.sh name:"$signal" "$file" | tail -n+2 | awk 'BEGIN{FS=OFS="\t"}{split($1, a, "_"); print a[1], $2}' | sort -k1,1 -k2,2gr | sort -u -k1,1 > tmp
...
/users/rg/bborsari/bin/add_header.sh "$outFile" > tmp
mv tmp "$outFile"
