...
for i,line in enumerate(file1):
	...
	#	h1 = "V0\t" + h1
	x_fields = len(list(line_sp)) - len(list(x_indeces))
...
for i,line in enumerate(file2):
	...
	if not args.b_header and args.a_header:
		if i == 0:
			h2 = "\t".join(("V"+str(n+1) for n in range(len(list(line_sp)))))
			print("%s\t%s" %(h2,h1))
	if not args.b_header and not args.a_header:
		if i == 0:
			h1 = "\t".join(("V"+str(n+1) for n in range(len(list(line_sp))+1, x_fields)))
			h2 = "\t".join(("V"+str(n+1) for n in range(len(line_sp))))
#			print("%s\t%s" %(h2,h1))
	if i==2 and args.b_header and len(list(line_sp))==len(list(h2.split("\t")))+1:
		h2 = "X0\t" + h1
