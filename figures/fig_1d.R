...
class <- unlist(sapply(1:5, function(y) combn(ordered_marks, y, paste, collapse = "+")))
names(class) <- as.character(unlist(sapply(1:5, function(y) combn(c(1, 2, 4, 8, 16), y, sum))))
...
names(color) <- as.character(unlist(sapply(1:5, function(y) combn(c(1, 2, 4, 8, 16), y, sum))))
...
## Plot proportion of cCREs from each category that have each combination of marks
barplot <- tmp[tmp$cCRE_id %in% m[m$ATAC == 'accessible', ]$cCRE_id, ]

### Add data on cCRE class
barplot <- merge(barplot, m[, c("cCRE_id", "supracategory")], 
                 by = "cCRE_id",
                 all.x = T)

barplot$H168 <- format(barplot$H168)
barplot[barplot$H168 %in% setdiff(unique(barplot$H168), 
                                  c("31", "15", " 7", " 3")), ]$H168 <- "minority"

barplot <- barplot %>%
  group_by(H168, supracategory) %>%
  dplyr::summarise(count = n()) %>%
  as.data.frame()

barplot$H168 <- factor(barplot$H168, levels = rev(c("31", "15", " 7", " 3", "minority")))

barplot$supracategory <- factor(barplot$supracategory,
                                levels = c("CA/TF",
                                           "dELS",
                                           "pELS",
                                           "PLS"))
color_barplot <- c(color[c("31", "15", " 7", " 3")],
                   "minority" = "grey")
class_barplot <- gsub("\\+", "\n\\+", class)
...
# Generate the plot only for cases marked by silent marks in at least one time point (only first and las time points)
barplot_silent_ss <- barplot_silent[apply(barplot_silent, 1, function(a) sum(a) > 0), ]
plot <- do.call(rbind, apply(barplot_silent_ss[intersect(m[m$ATAC == 'accessible', ]$cCRE_id, 
                                                         rownames(barplot_silent_ss)), 
                                               c('H000', 'H168')], 1, function(x) {
                                                 data.frame(x = names(x), node = x, 
                                                            next_x = dplyr::lead(names(x)), 
                                                            next_node = dplyr::lead(x), row.names = NULL)
                                               })) %>%
  mutate(x = factor(x, names(tmp[, c('H000', 'H168')])),
         next_x = factor(next_x, names(tmp[, c('H000', 'H168')])))
