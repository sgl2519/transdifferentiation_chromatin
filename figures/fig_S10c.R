...
x <- read.table("../analysis/expression/629.expressed.0h.txt",
                stringsAsFactors = F)

# Load matrix of H3K27me3 marking
h3k27me3 <- read.table('../analysis/all.marks/H3K27me3/QN.merged/ant.del.analysis/H3K27me3.peaks.dynamics.tsv')
...
# 2. read set of 629 already expressed genes
x <- read.table("../analysis/expression/629.expressed.0h.txt",
                stringsAsFactors = F)
colnames(x) <- "gene_id"
...
for ( i in 1:9) {
  ...
  # 5.4. for each of the 629 already expressed genes
...
# Order using seriation H3K27me3 pre-marked and not pre-marked independently
ordered <- data.frame()
for ( names in list(act.genes.unmarked, act.genes.premarked) ) {
  tmp <- x[names, ]
  ser.obj <- seriate(as.matrix(tmp), method = "PCA_angle")
  x.ser <- tmp[get_order(ser.obj, 1), ]
