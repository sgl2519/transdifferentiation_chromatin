...
# 10. add a state "unmarked" for
# those genes that are unmarked at time-point 0h
# for all marks
unmarked.genes <- setdiff(unique(x.melt.agg$gene_id), (x.melt.agg[x.melt.agg$value == "H000", "gene_id"]))
x.melt.agg <- rbind(x.melt.agg, 
                    data.frame(gene_id = unmarked.genes,
                               value = rep("H000", length(unmarked.genes)),
                               variable = rep("unmarked", length(unmarked.genes))))
...
root.gs <- c("unmarked", "H3K4me1;H3K4me2",
             "H3K27ac;H3K9ac", "expression",
...
correspondence.v <- c()
names.correspondence.v <- c("start", "unmarked", "H3K4me1;H3K4me2",
                            "H3K27ac;H3K9ac", "expression",
                            "H3K4me3;H3K36me3", "H4K20me1")
...
pdf("fig_S10d_left.arcs_increase.pdf", 
    height = 5, width = 10)
...
pdf("fig_S10d_left.arcs_decrease.pdf", 
    height = 5, width = 10)
arcDiagram(as.matrix(counts.df[counts.df$i_state > counts.df$j_state, 2:3]),
           lwd = (as.numeric(counts.df[counts.df$i_state > counts.df$j_stat, "n"]/5)))
dev.off()
