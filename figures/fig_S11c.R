...
# 2. read set of 629 activated genes
x <- read.table("expression/629.expressed.0h.txt",
                stringsAsFactors = F)
colnames(x) <- "gene_id"
...
for ( i in 1:9) {
  ...
  # 5.4. for each of the 629 activated genes
...
# setting 11 = tp at which expression reaches 25% of upregulation
...
tp.m <- read.table("expression/expression.0.25.increases.tsv",
                   h=F, sep="\t")
colnames(tp.m) <- c("gene_id", "25%")
tp.m <- tp.m[tp.m$gene_id %in% rownames(expression.matrix), ]
...
rownames(m) <- gene_ids
# we lose 6 genes: 5 start from > 25%, and one is upreg at 168 
# these genes are removed because they would not have a before and after heatmap
m <- m[m$H168_expression == 0, ] 

x <- merge(x, m, by = 0, all.y = T)
...
# one gene is removed because it gets expressed at 168h, so we wouldn't have the time-point after gene expression upreg.
...
x1.ser.promoters <- x1[get_order(ser.obj1, 1), ]
...
x2.ser.promoters <- x2[get_order(ser.obj2, 1), ]
...
# 3. read set of 629 already expressed genes
x <- read.table("../../../all.marks/expression/629.expressed.0h.txt",
                stringsAsFactors = F)
colnames(x) <- "gene_id"
...
# of presence/absence of peaks across time-points
# for the 629 already expressed genes
for ( i in 1:9) {
  ...
  # 6.4. for each of the 629 already expressed genes
  for (gene in x$gene_id) {
...
x <- x[rownames(x) %in% rownames(expression.tp), ]
...
for ( i in 1:9) {
  ...
  # 7.2. keep from tmp only the 629 already expressed genes
...
genes.pairs <- read.delim("/no_backup/rg/bborsari/projects/ERC/human/2018-01-19.chip-nf/Borsari_et_al/analysis/enhancers/ABC.genewise.blood.closest/ELS_gene_pairs_genewise_blood_closest.bed",
                          h=F)
...
rm(list=setdiff(ls(), c("x1.ser.promoters", "x2.ser.promoters",
...
# before expression
h <- Heatmap(rbind(x1.ser.promoters[1:559, c(1:9)], x2.ser.promoters[1:64, c(1:9)]), 
             column_title = "before\nexpression",
  ...
  # with expression
  Heatmap(rbind(x1.ser.promoters[1:559, c(10:18)], x2.ser.promoters[1:64, c(10:18)]),
          column_title = "with\nexpression",
          column_title_gp = gpar(fontsize = 10),
          name = "promoter 2", 
          cluster_rows = F,
  ...
  # after expression
  Heatmap(rbind(x1.ser.promoters[1:559, c(19:27)], x2.ser.promoters[1:64, c(19:27)]),
          column_title = "after\nexpression",
          column_title_gp = gpar(fontsize = 10),
          name = "promoter 3", 
          cluster_rows = F,
...
# before expression
h <- Heatmap(rbind(x1.ser.proxEnhancers[1:559, c(1:9)], x2.ser.proxEnhancers[1:64, c(1:9)]), 
             column_title = "before\nexpression",
             column_title_gp = gpar(fontsize = 10),
  ...
  # with expression
  Heatmap(rbind(x1.ser.proxEnhancers[1:559, c(10:18)], x2.ser.proxEnhancers[1:64, c(10:18)]),
          column_title = "with\nexpression",
          column_title_gp = gpar(fontsize = 10),
  ...
  # after expression
  Heatmap(rbind(x1.ser.proxEnhancers[1:559, c(19:27)], x2.ser.proxEnhancers[1:64, c(19:27)]),
          column_title = "after\nexpression",
          column_title_gp = gpar(fontsize = 10),
...
# before expression
h <- Heatmap(rbind(x1.ser.distEnhancers[1:559, c(1:9)], x2.ser.distEnhancers[64:1, c(1:9)]), 
             column_title = "before\nexpression",
             column_title_gp = gpar(fontsize = 10),
  ...
  # with expression
  Heatmap(rbind(x1.ser.distEnhancers[1:559, c(10:18)], x2.ser.distEnhancers[64:1, c(10:18)]),
          column_title = "with\nexpression",
          column_title_gp = gpar(fontsize = 10),
  ...
  # after expression
  Heatmap(rbind(x1.ser.distEnhancers[1:559, c(19:27)], x2.ser.distEnhancers[64:1, c(19:27)]), 
          column_title = "after\nexpression",
          column_title_gp = gpar(fontsize = 10),
