...
function1 <- function(mark) {
  ...
  print(length(unique(setdiff(intersect(not.expressed.genes, marked.genes), mark.sig.genes))))
...
for ( i in 1:9 ) {
  ...
  y <- c(y, paste0(marks[i], ': ', nrow(tmp1)))
...
for ( i in 1:9 ) {
  ...
  mark.partition <- c(mark.partition, rep(as.character(y[i]), str_split_fixed(string = y[i],
                                                                              pattern = ': ',
                                                                              n = 2)[2]))
