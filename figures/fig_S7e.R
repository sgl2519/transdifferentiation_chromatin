...
function1 <- function(mark, x="pearson") {
  ...
  # 1. read mark matrix
  m <- read.table(paste0("../enhancers/proximal.enhancers.first.TSS/all.marks/",
                         mark, "/QN.merged/", mark, ".matrix.after.QN.merged.tsv"),
                  h=T, sep="\t")
  
  rownames(m) <- gsub("c.*", "", rownames(m))
  
  # 2. obtain the set of genes for which there is an associated distal enhancer
  ss <- intersect(rownames(e), rownames(m))
  m <- m[ss, ]
  e_ss <- e[ss, ]
  
  stopifnot(identical(rownames(e_ss), rownames(m)))
  
  # 3. compute correlation for each gene between expression and mark profiles
  y <- diag(cor(t(e_ss), t(m), method=x))
  
  # 4. prepare df
  df <- data.frame(cc=y)
  rownames(df) <- rownames(e_ss)
  df$mark <- mark
  df$type <- "across\ntime"
  df$tp <- NA
